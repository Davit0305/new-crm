        ...
        <div class="hide_line searchall" id="formallsearch">
            ...
            <form v-else>
                ...
                <div style="margin: 15px 0 0 15px;" class="exelsearch">
                    <img v-if="exelLoad === false" @click="exportWorkers()" src="/icons/exel.png" width="24" height="24">
...
<script>
    export default {
        ...
        data: function () {
            return {
                filter: {
                    enabled: this.$cookies.get("workerFilter_enabled") ? this.$cookies.get("workerFilter_enabled") : 0,
                    job: this.$cookies.get("workerFilter_job") ? this.$cookies.get("workerFilter_job") : 0,
                    dep: this.$cookies.get("workerFilter_dep") ? this.$cookies.get("workerFilter_dep") : 0,
        ...
        mounted() {
            ...
            this.$root.$on('clear-worker-search-form', () => {
                this.clearFilter()
            });
        },
        methods: {
            ...
            getSearchModal() {
                axios.post('/api/worker/getWorkersSearchData')
                    .then( res => {
                        // console.log(res);
            ...
            exportWorkers() {
                ...
                if(this.filter.enabled != 0) {
                    this.$cookies.set("workerFilter_enabled", this.filter.enabled);
                } else {
                ...
                if(this.filter.job != 0) {
                    this.$cookies.set("workerFilter_job", this.filter.job);
                } else {
                    this.$cookies.remove("workerFilter_job");
                }
                if(this.filter.dep != 0) {
                    this.$cookies.set("workerFilter_dep", this.filter.dep);
                } else {
                    this.$cookies.remove("workerFilter_dep");
                }
                axios.get('/api/worker/exportWorkers', {responseType: 'blob'})
                    .then( resp => {
                        // console.log(resp);
